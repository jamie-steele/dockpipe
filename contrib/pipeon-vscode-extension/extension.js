 ...
 * @see docs/pipeon-vscode-fork.md
 */
// @ts-check
...
function activate(context) {
    ...
    vscode.commands.registerCommand("pipeon.openContextBundle", async () => {
      ...
      try {
        ...
        vscode.window.showInformationMessage(
          "Pipeon: no .dockpipe/pipeon-context.md — run `bin/pipeon bundle` (see scripts/pipeon/README.md) or generate artifacts first."
        );
      }
    })
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("pipeon.showReadme", async () => {
      ...
      const doc = vscode.Uri.joinPath(root, "docs", "pipeon-vscode-fork.md");
      try {
        ...
        vscode.window.showInformationMessage(
          "Pipeon: docs/pipeon-vscode-fork.md not found in this workspace — open the dockpipe repo root."
        );
      }
    })
  );
