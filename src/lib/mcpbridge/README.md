# `mcpbridge` — MCP stdio bridge (dumb interface)
...
- read-only discovery via `dockpipe/src/lib/dockpipe/infrastructure` (workflow names only).
...
- `src/cmd/mcpd` — stdio MCP (default), or **HTTPS/HTTP** when `-http` / `MCP_HTTP_LISTEN` is set.
...
- [docs/mcp-architecture.md](../../../docs/mcp-architecture.md) — boundaries and tool table  
- [docs/mcp-agent-trust.md](../../../docs/mcp-agent-trust.md) — artifacts vs MCP, exec gate, dogfooding **`.cursor/mcp.json`**  
- [docs/mcp-host-hardening.md](../../../docs/mcp-host-hardening.md) — host / `PATH` / workdir hardening
