...
func TestListWorkflowNamesInRepoRootIncludesDockpipeWorkflows(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(tmp, "shipyard", "workflows", "local"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(tmp, "shipyard", "workflows", "local", "config.yml"), []byte("name: local\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	got, err := ListWorkflowNamesInRepoRoot(tmp)
	if err != nil {
...
func TestResolveWorkflowConfigPathPrefersAuthoringDockpipeWorkflowsOverTemplates(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(tmp, "shipyard", "workflows", "demo"), 0o755); err != nil {
		t.Fatal(err)
	}
	localWf := filepath.Join(tmp, "shipyard", "workflows", "demo", "config.yml")
