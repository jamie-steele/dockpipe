...
// that contain a resolver profile directory. Also includes src/lib/dorkpipe/workflows/<leaf> when present.
func nestedResolverLeafDirs(repoRoot, leaf string, roots []string) []string {
	leaf = strings.TrimSpace(leaf)
	if leaf == "" {
		return nil
	}
	var out []string
	dp := filepath.Join(DorkpipeLibraryWorkflowsDir(repoRoot), leaf)
...
func nestedResolverProfileCandidates(repoRoot, name string, roots []string) []string {
	...
	for _, base := range nestedResolverLeafDirs(repoRoot, name, roots) {
...
// Tries src/lib/dorkpipe/workflows, nested src/core/workflows/**/<name>, then each extra root /**/<name>.
func nestedWorkflowConfigCandidates(repoRoot, name string, roots []string) []string {
	...
	add(filepath.Join(DorkpipeLibraryWorkflowsDir(repoRoot), name, "config.yml"))
