...
// materializes a workflow config (under BundledLayoutDir/workflows/ — cache layout, not a dependency on
...
func TestRepoRootMaterializesBundledTemplates(t *testing.T) {
	...
	cfg := filepath.Join(got, BundledLayoutDir, "workflows", "run", "config.yml")
	st, err := os.Stat(cfg)
	if err != nil || st.IsDir() {
