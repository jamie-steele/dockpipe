...
| Subfolder | Contents |
|-----------|----------|
| **`scripts/`** | **Agnostic** helpers at this folder root only. **Domain** bundles (**`dorkpipe/`**, **`pipeon/`**, …) live under **`../bundles/`** (downstream); **this repo** also keeps **`review-pipeline`** under repo-root **`workflows/review-pipeline/`**. Resolver-only host scripts live under **`../resolvers/<name>/`**. See **`scripts/README.md`**, **`bundles/README.md`**. |
