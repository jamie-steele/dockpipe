...
| **`packages/cloud/storage/.../terraform-cloudflare-r2-run.sh`** | host | Cloudflare R2 + bundled r2publish module: **`scripts/dockpipe.cloudflare.r2infra/terraform-cloudflare-r2-run.sh`** (resolver **`dockpipe.cloudflare.r2infra`**). Used by **`r2-publish.sh`** too (not **`terraform-run.sh`**). |
