...
# Compose file: src/templates/core/bundles/dorkpipe/assets/compose/docker-compose.yml
# When done: dev-stack.sh down — containers stop (nothing long-lived required for orchestration).
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../../.." && pwd)"
COMPOSE="$REPO_ROOT/src/templates/core/bundles/dorkpipe/assets/compose/docker-compose.yml"
if [[ ! -f "$COMPOSE" ]]; then
