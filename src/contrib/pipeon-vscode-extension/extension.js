 ...
 * @see src/apps/pipeon/docs/pipeon-vscode-fork.md
 */
// @ts-check
...
function activate(context) {
    ...
    vscode.commands.registerCommand("pipeon.openContextBundle", async () => {
      ...
      try {
        ...
        vscode.window.showInformationMessage(
          "Pipeon: no .dockpipe/pipeon-context.md — run `src/bin/pipeon bundle` (see src/apps/pipeon/scripts/README.md) or generate artifacts first."
        );
      }
    })
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("pipeon.showReadme", async () => {
      ...
      const doc = vscode.Uri.joinPath(root, "src", "apps", "pipeon", "docs", "pipeon-vscode-fork.md");
      try {
        ...
        vscode.window.showInformationMessage(
          "Pipeon: src/apps/pipeon/docs/pipeon-vscode-fork.md not found in this workspace — open the dockpipe repo root."
        );
      }
    })
  );
