...
**Bundled Pipeon shell** (**`pipeon.sh`**, **`chat.sh`**, **`bundle-context.sh`**, **`lib/`**, **`prompts/`**) is canonical under **`.staging/packages/dockpipe/ide/pipeon/assets/scripts/`**; this directory repeats those paths as **symlinks** for stable **`src/apps/pipeon/scripts/…`** references (same idea as **`scripts/dorkpipe/`**).
