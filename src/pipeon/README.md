...
**Pipeon** is separate from the DockPipe primitive: local-first assistant UI, **Code OSS fork** story, VS Code extension, and the **`src/bin/pipeon`** harness (repo root). Everything for that lives under **`src/pipeon/`** so top-level **`docs/`** stays focused on DockPipe / DorkPipe / workflow contracts.
