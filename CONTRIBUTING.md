...
**Bundled layout in docs/code:** framework scripts, images, and compose live under **`templates/core/assets/`** only (`assets/scripts`, `assets/images`, `assets/compose`). After editing docs or path strings, run **`make check-paths`** (or **`bash scripts/check-templates-core-paths.sh`**) to catch drift toward removed paths (pre-`assets/` layout or old `workflows` / `run-worktree` trees). CI runs this check.
...
**Isolation profiles** (resolver files): shared definitions under **`templates/core/resolvers/<name>`** only — not beside individual workflows. See **[docs/isolation-layer.md](docs/isolation-layer.md)**. New Dockerfile-backed name → **`templates/core/assets/images/<name>/Dockerfile`** + a branch in **`lib/dockpipe/infrastructure/template.go`** (`TemplateBuild`). See **[docs/templates-core-assets.md](docs/templates-core-assets.md)**. **Framework scripts:** shared run/act/pre scripts belong in **`templates/core/assets/scripts/`**; workflow YAML still references them as **`scripts/…`** (resolved to project `scripts/` or **`templates/core/assets/scripts/`**). Repo-only tooling stays in top-level **`scripts/`** (see **`scripts/README.md`**).
...
**Action:** add **`templates/core/assets/scripts/<name>.sh`** for bundled copies; use `DOCKPIPE_EXIT_CODE`, `DOCKPIPE_CONTAINER_WORKDIR`. Add to `action init --from` list in **`cmd/dockpipe`** if copyable.
