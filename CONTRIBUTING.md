...
**Action:** add **`templates/core/assets/scripts/<name>.sh`** for agnostic bundled actions; domain-specific assets go under **`templates/core/bundles/`** when appropriate. Use `DOCKPIPE_EXIT_CODE`, `DOCKPIPE_CONTAINER_WORKDIR`. Add to `action init --from` list in **`cmd/dockpipe`** if copyable.
