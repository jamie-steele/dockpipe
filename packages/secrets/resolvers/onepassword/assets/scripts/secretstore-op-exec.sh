#!/usr/bin/env bash
# 1Password CLI: op run --env-file — maintainer staging (not bundled core secretstore).
# Resolved via scripts/dockpipe/… → .staging/workflows/dockpipe/assets/scripts/ when not under src/scripts/.
set -euo pipefail
...
case "$PROVIDER" in
  op)
    ...
    [[ -f "$OP_ENV_FILE" ]] || die "missing $OP_ENV_FILE — copy .staging/workflows/dockpipe/packages/secrets/resolvers/onepassword/.env.op.template.example to .env.op.template and add op:// lines"
