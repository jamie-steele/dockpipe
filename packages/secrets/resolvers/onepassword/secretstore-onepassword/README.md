...
Maintainer-only **`workflow_type: secretstore`** flow using the **1Password CLI** (`op run --env-file`). Same shape as bundled **`secretstore`** (dotenv file), with a **`kind: host`** step that runs **`scripts/onepassword/secretstore-op-exec.sh`** from this package.
