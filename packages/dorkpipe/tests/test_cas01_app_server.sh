...
if go run "$HARNESS" --mode policy --model '' >/dev/null 2>&1; then echo "CAS-01 accepted an empty model" >&2; exit 1; fi
if go run "$HARNESS" --mode policy --reasoning-effort '' >/dev/null 2>&1; then echo "CAS-01 accepted an empty reasoning effort" >&2; exit 1; fi
