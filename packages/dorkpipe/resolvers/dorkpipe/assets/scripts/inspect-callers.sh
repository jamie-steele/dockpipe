...
if [[ -z "$SYMBOL" ]]; then
  cat <<'EOF'
No callable symbol could be inferred.

Try:
- /callers myFunction
- select the function name first
EOF
  exit 0
fi

echo "Target callable: $SYMBOL"
[[ -n "$ACTIVE_FILE" ]] && echo "Active file: $ACTIVE_FILE"
echo

echo "Likely callers:"
rg -n --hidden -g '!**/.git/**' -g '!**/node_modules/**' -g '!**/target/**' -g '!**/.dockpipe/**' -g '!**/.dorkpipe/**' \
  "\b${SYMBOL}\s*\(" . | sed -n '1,40p' || true
echo

echo "Method-style callers:"
rg -n --hidden -g '!**/.git/**' -g '!**/node_modules/**' -g '!**/target/**' -g '!**/.dockpipe/**' -g '!**/.dorkpipe/**' \
  "\.${SYMBOL}\s*\(" . | sed -n '1,40p' || true
