...
HAS_BRANCH=0
if [[ -f "$ROOT/packages/dorkpipe/lib/examples/full-bar.yaml" ]]; then
	HAS_BRANCH=1
fi
...
EOF
	...
	echo "| CLI | \`packages/dorkpipe/lib/cmd/dorkpipe/main.go\` |"
	echo "| Eval / promotion | \`lib/dorkpipe/eval/eval.go\`, \`lib/dorkpipe/promotion/promotion.go\` |"
	echo "| DockPipe workflow (this integration) | \`packages/dorkpipe/resolvers/dorkpipe-self-analysis/\` |"
	echo "| Orchestrator example DAG | \`packages/dorkpipe/resolvers/dorkpipe-orchestrator/spec.example.yaml\` |"
	echo ""
	...
	echo "- **No** vendor-specific logic in \`src/lib/application\` / runner for this work."
	...
	echo "4. Document the new path in \`packages/dorkpipe/resolvers/dorkpipe-self-analysis/README.md\` if behavior is user-visible."
	echo ""
...
{
	cat <<EOF
You are working in the DockPipe repository. Follow AGENTS.md: DockPipe stays the primitive; DorkPipe (packages/dorkpipe/lib/, dorkpipe/bin/dorkpipe) is orchestration on top — no second hidden engine.

Repository snapshot (machine-generated):
- ref: ${HEAD_REF}
- HEAD: ${HEAD_SHA}
- dorkpipe.orchestrator Go files: ${GO_N}
...
1) If this is a first pass: read packages/dorkpipe/lib/engine/run.go and packages/dorkpipe/lib/spec/spec.go to see current policy and scheduling.
2) Add or extend a shipped DAG spec (packages/dorkpipe/resolvers/dorkpipe-orchestrator/spec.example.yaml) so dogfood matches engine capabilities (e.g. kind: verifier after local LLM, or one pattern from packages/dorkpipe/lib/examples/full-bar.yaml).
3) Add tests under packages/dorkpipe/lib/*/ when you change aggregation, planner, or workers.
4) Run: make build && go test ./... && packages/dorkpipe/bin/dorkpipe validate -f packages/dorkpipe/resolvers/dorkpipe-self-analysis/spec.yaml

Constraints:
- No vendor-specific hooks in src/lib/application for this work.
...
EOF
	if [[ "$HAS_VERIFIER" -eq 0 ]]; then
		echo "Priority: orchestrator spec.example.yaml still lacks a kind: verifier node after local_reason — add one so Vector.Verifier is populated before escalation (packages/dorkpipe/lib/workers/workers.go)."
		echo ""
	fi
	cat <<'EOF'
Start here (files):
...
- packages/dorkpipe/resolvers/dorkpipe-orchestrator/spec.example.yaml
- packages/dorkpipe/lib/examples/full-bar.yaml
