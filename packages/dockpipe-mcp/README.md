# `dockpipe-mcp` (`dockpipe.mcp`)

Maintainer package: **MCP bridge** for DockPipe / DorkPipe — Go module **`dockpipe.mcp`**, library **`mcpbridge/`**, binary **`cmd/mcpd`**.
...
The **`mcpd`** binary is built into **`dockpipe-mcp/bin/mcpd`** (next to this module). It is **not** copied under **`src/bin/`** — point Cursor / env at this path (see **`src/Makefile`**).
...
After **`make maintainer-tools`**, point **`command`** at **`dockpipe-mcp/bin/mcpd`**. Default tier without env is **`validate`**; use **`DOCKPIPE_MCP_TIER=readonly`** for the smallest surface.
