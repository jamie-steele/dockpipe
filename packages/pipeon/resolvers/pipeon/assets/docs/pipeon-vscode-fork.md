...
| **Extension-only (no fork)** | Ship **`packages/pipeon/resolvers/pipeon/vscode-extension`** into **stock VS Code**—fastest path for dev; **not** a branded Pipeon desktop by itself. |
...
| Addition | Purpose |
|----------|---------|
| **Bundled or recommended extension** | Ship **`packages/pipeon/resolvers/pipeon/vscode-extension`** from **this** repo as a `.vsix` or built-in extension folder. |
...
```bash
cd /path/to/dockpipe/packages/pipeon/resolvers/pipeon/vscode-extension
npm install
