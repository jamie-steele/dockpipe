 ...
 */
import type { ExtensionContext, LogOutputChannel, Webview, WebviewPanel, WebviewView } from "vscode";
...
const USER_STUDIO_STATE_KEY = "pipeon.userStudioState.v1";
...
function sanitizeYamlId(value, fallback = "dorkpipe-agentic-flow") {
...
function yamlBlock(lines, indent) {
  ...
  const normalized = text.trim() || "Generated by the DorkPipe template designer.";
...
function yamlStringList(values, indent) {
  ...
  const pad = " ".repeat(indent);
  return `\n${items.map((item) => `${pad}- ${yamlScalar(item)}`).join("\n")}`;
}

function flattenTemplateNodes(nodes, out: any[] = []) {
  for (const node of Array.isArray(nodes) ? nodes : []) {
    out.push(node);
    if (node?.type === "loop") {
      flattenTemplateNodes(node.children, out);
    }
  }
  return out;
}

function modelEntryById(modelStore, id) {
  const entries = normalizeModelStore(modelStore).entries;
  return entries.find((entry) => entry.id === id) || entries[0] || normalizeModelEntry(null);
}

function templateNodeToTask(node, modelStore, index) {
  const type = String(node?.type || "dockpipe").toLowerCase();
  const model = type === "model" ? modelEntryById(modelStore, node?.config?.modelId) : null;
  const label = String(node?.label || node?.id || `Task ${index + 1}`);
  const task = {
    id: sanitizeYamlId(node?.id || label, `task-${index + 1}`),
    workerType: type === "model" ? "model" : type === "loop" ? "control" : "dorkpipe",
    resolverHint: type === "model" ? model?.provider || DEFAULT_MODEL_PROVIDER : "dorkpipe",
    goal: label,
    expectedOutput: String(node?.config?.output || node?.config?.artifactMode || "artifact-or-answer"),
    prompt: [
      node?.decision ? `Decision: ${node.decision}` : "",
      node?.notes ? `Notes: ${node.notes}` : "",
      node?.guidance?.orchestration ? `Orchestration guidance: ${node.guidance.orchestration}` : "",
      node?.guidance?.model ? `Model guidance: ${node.guidance.model}` : "",
      type === "dockpipe" && node?.config?.phase ? `DorkPipe phase: ${node.config.phase}` : "",
      type === "loop" && node?.config?.stopCondition ? `Loop stop condition: ${node.config.stopCondition}` : "",
      type === "model" && node?.config?.task ? `Model task: ${node.config.task}` : "",
    ].filter(Boolean).join("\n"),
    maxCloudTokens: type === "model" && model?.provider !== DEFAULT_MODEL_PROVIDER ? 20000 : 0,
    model: model ? {
...
function renderDesignerTemplateWorkflowYaml(template, modelStore) {
  const normalized = normalizeReasoningTemplate(template, modelStore, template || createBlankReasoningTemplate(modelStore));
  const id = sanitizeYamlId(normalized.name || normalized.id);
  const nodes = flattenTemplateNodes(normalized.nodes);
  const tasks = nodes.map((node, index) => templateNodeToTask(node, modelStore, index));
  const maxLoopIterations = Math.max(
    1,
    ...nodes
      .filter((node) => node?.type === "loop")
      .map((node) => Number(node?.config?.maxIterations || 1) || 1)
  );
  const promptParts = [
    normalized.description,
    normalized.globalModifiers?.safetyRules ? `Safety rules:\n${normalized.globalModifiers.safetyRules}` : "",
    normalized.globalModifiers?.outputRequirements ? `Output requirements:\n${normalized.globalModifiers.outputRequirements}` : "",
    normalized.globalModifiers?.executionConstraints ? `Execution constraints:\n${normalized.globalModifiers.executionConstraints}` : "",
    normalized.guidance?.orchestration ? `Orchestration guidance:\n${normalized.guidance.orchestration}` : "",
    normalized.guidance?.model ? `Model guidance:\n${normalized.guidance.model}` : "",
  ];

  const lines = [
    `name: ${yamlScalar(id)}`,
    `description: ${yamlScalar(normalized.description || "DorkPipe agentic workflow generated from the visual designer.")}`,
    "model_policy:",
    "  mode: escalate",
    "  attempt:",
    `    preference: ${yamlScalar(normalized.globalModifiers?.routingPreference || "local-first")}`,
    `    max_attempts: ${maxLoopIterations}`,
    "  validate:",
    "    preference: strong",
    "    required_for:",
    "      - merge",
    ...
    "steps:",
    "  - id: agentic-flow",
    `    name: ${yamlScalar(normalized.name || "DorkPipe Agentic Flow")}`,
    "    kind: host",
    "    agent:",
    "      startup_prompt: |",
    yamlBlock(promptParts, 8),
    ...
    "      orchestration:",
    "        request:",
    `          text: ${yamlScalar(normalized.description || normalized.name || "Run the DorkPipe agentic flow.")}`,
    "        plan:",
    `          goal: ${yamlScalar(normalized.name || "DorkPipe Agentic Flow")}`,
    `          steps:${yamlStringList(tasks.map((task) => task.goal), 12)}`,
    "        tasks:",
  ];

  for (const task of tasks.length ? tasks : [templateNodeToTask(makeDefaultNodeForType("dockpipe", modelStore), modelStore, 0)]) {
    lines.push(`          - id: ${yamlScalar(task.id)}`);
    lines.push(`            worker_type: ${yamlScalar(task.workerType)}`);
    lines.push(`            resolver_hint: ${yamlScalar(task.resolverHint)}`);
    lines.push(`            goal: ${yamlScalar(task.goal)}`);
    lines.push(`            expected_output: ${yamlScalar(task.expectedOutput)}`);
    lines.push("            prompt: |");
    lines.push(yamlBlock(task.prompt, 14));
    if (task.maxCloudTokens) {
      lines.push(`            max_cloud_tokens: ${task.maxCloudTokens}`);
    }
    if (task.model) {
      lines.push("            model:");
      lines.push(`              provider: ${yamlScalar(task.model.provider)}`);
      lines.push(`              model: ${yamlScalar(task.model.model)}`);
      lines.push(`              num_ctx: ${Math.max(1024, Number(task.model.numCtx) || 8192)}`);
    }
  }

  lines.push("        merge:");
  lines.push("          id: merge");
  lines.push("          strategy: synthesize");
  lines.push("        verify:");
  lines.push("          commands:");
  lines.push("            - ./src/bin/dockpipe workflow validate ${DOCKPIPE_WORKFLOW_CONFIG}");
  lines.push("");
  return lines.join("\n");
}
...
function describeRequestEvent(label, metadata: AnyRecord = {}) {
...
function normalizeUserStudioState(raw, fallback?) {
  const base = createInitialChatState();
...
function compactUserStudioStateForPersistence(chatStore) {
  const safe = ensureValidChatStore(chatStore);
  return {
    activeTemplateId: templateById(safe.reasoningTemplates, safe.activeTemplateId)?.id || BUILTIN_TEMPLATE_ID,
    modelStore: {
...
function logChannelInfo(channel, message, data?: unknown) {
  if (!channel) {
    return;
  }
...
// Canonical runtime renderer: this is the shell Pipeon actually serves.
...
class PipeonChatViewProvider {
  ...
  constructor(context: ExtensionContext, channel: LogOutputChannel, extensionVersion?: string) {
  ...
  async persistChatStore() {
    ...
    try {
      await this.context.globalState.update(USER_STUDIO_STATE_KEY, userStudio);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      this.channel.error(`Failed to persist user template/model state: ${message}`);
    }
    try {
  ...
  async exportTemplateYaml(template) {
    const active = template && typeof template === "object"
      ? template
      : templateById(this.chatStore.reasoningTemplates, this.chatStore.activeTemplateId);
    const normalized = normalizeReasoningTemplate(
      active || createDefaultReasoningTemplate(this.chatStore.modelStore),
      this.chatStore.modelStore,
      active || undefined
    );
    const yaml = renderDesignerTemplateWorkflowYaml(normalized, this.chatStore.modelStore);
  ...
  bindWebviewSurface(surfaceId, webview) {
    ...
    webview.onDidReceiveMessage(async (msg) => {
      try {
        ...
        if (msg?.type === "exportTemplateYaml") {
          await this.exportTemplateYaml(msg.template);
          return;
        }
