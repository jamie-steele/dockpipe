...
3. **Workflow YAML** — **`capability:`** identifies the workflow (and may match a resolver **`package.yml`**). If **`resolver:`** / **`default_resolver:`** are omitted, the runner looks up **`templates/core/resolvers/<name>`** from resolver packages whose **`package.yml`** declares the same **`capability:`**. **`dockpipe.*`** capabilities can imply a default **`runtime:`** substrate (e.g. **`dockpipe.docker`** → **`dockerimage`**) when **`runtime:`** is unset. **`runtime:`** / explicit **`resolver:`** still override when set.
