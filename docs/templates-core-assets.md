...
| **`scripts/dorkpipe/`**, **`scripts/pipeon/`** → **`src/pipeon/scripts/`**, **`scripts/review-pipeline/`**, … | **SAFE TO BUNDLE** | **Domain asset packs** — **not** DockPipe resolvers; canonical tree **`templates/core/bundles/<domain>/`** (runner maps **`scripts/…`** there). **`scripts/pipeon`** is a symlink to **`src/pipeon/scripts/`**. |
