...
| **`scripts/dorkpipe/`**, **`scripts/pipeon/`** → **`src/apps/pipeon/scripts/`**, **`scripts/review-pipeline/`**, … | **SAFE TO BUNDLE** | **Domain asset packs** under **`.staging/bundles/<domain>/`** (runner maps **`scripts/…`** there). Pipeon harness + docs: **`src/apps/pipeon/`**. |
