# TASK-011 Local Windows CI VM Parity And Guest Tooling
...
- Add an explicit coverage split for managed workspace volume bootstrap:
  - one test for git-aware bootstrap (`session.volume.seed`)
  - one test for checkout/worktree-style bootstrap (`session.volume.sync_in`)
