...
| **`DOCKPIPE_RESOLVER_WORKFLOW`** | no | Bundled delegate YAML under **`templates/core/resolvers/<name>/config.yml`** (or **`shipyard/core/resolvers/<name>/config.yml`** when materialized). **Mutually exclusive** with **`DOCKPIPE_RESOLVER_HOST_ISOLATE`**. |
