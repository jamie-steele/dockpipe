#!/usr/bin/env bash
# Test that --action actions/commit-worktree.sh resolves to the bundled action when run from outside the repo.
set -euo pipefail
...
# Run from $tmp with relative action path (must resolve to bundled examples/actions/commit-worktree.sh)
"$DOCKPIPE" --workdir "$tmp" --action actions/commit-worktree.sh \
