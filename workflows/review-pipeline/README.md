...
| **`emulate-ci-host.sh`** | Quick host-only emulator for the repo's scan + Go test subset; useful for fast reproduction when you do not want the full **`ci-emulate`** / **`make ci`** path. |
...
**Workflow integration:** **`test-demo`** uses **`isolate: ollama`** so dockpipe **builds** the image via **`TemplateBuild`** (same as `codex`, `claude`, …). Merge keys into **`.dockpipe/outputs.env`** as usual. For a **host-only** Ollama (no dockpipe image), use **`kind: host`** + **`run: …/optional-local-model-summary.sh`** instead.
