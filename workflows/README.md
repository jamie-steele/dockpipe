...
| **`package-store-infra`** | Thin composer: shared **`vars`** + nested **`dockpipe.cloudflare.r2infra`** (`runtime: package`); optional **`--tf`**. Store tarballs: run **`dockpipe package build store`** separately when needed. |

**Suggested local “full stack”:** run **`make`** / **`go test`**, then **`bash src/scripts/ci-local.sh`** (host scans + **`dockpipe --workflow test`**), optionally **`./src/bin/dockpipe --workflow dockpipe-repo-quality --workdir . --`**. For Cloudflare R2 infra with repo **`vars`**: **`./src/bin/dockpipe --workflow package-store-infra --workdir . --`**. For store tarballs: **`dockpipe package build store`** after compile.
