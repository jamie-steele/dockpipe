...
| **`package/package-store-publish`** | Maintainer mirror workflow: `vault: op` → build release artifacts → upload each file to R2/S3 with `dockpipe release upload`. Dry-run by default. |

**Suggested local “full stack”:** run **`make ci`** or **`./src/bin/dockpipe --workflow ci-emulate --workdir . --`** to execute the Linux job locally through **`act`** and the Windows subset through the native host or **`windows-vm`**, depending on platform. On Linux, assume the configured Windows VM image is already provisioned for SSH. Optionally follow with **`./src/bin/dockpipe --workflow dockpipe-repo-quality --workdir . --`** to inspect normalized findings. For Cloudflare R2 infra with repo **`vars`**: **`./src/bin/dockpipe --workflow package-store-infra --workdir . --`**. For package mirror artifacts + R2 upload: **`./src/bin/dockpipe --workflow package-store-publish --workdir . --`**.
