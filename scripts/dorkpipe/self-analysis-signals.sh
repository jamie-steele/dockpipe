...
engine_files() {
	if command -v rg >/dev/null 2>&1; then
		rg -l 'branch_judge|retrieve_if|EarlyStop|verifier|ShouldEscalate|mergeVectors|dorkpipe\.metrics' \
			"$ROOT/lib/dorkpipe" "$ROOT/dockpipe/workflows" "$ROOT/scripts/dorkpipe" 2>/dev/null | sort -u | head -80 || true
	else
...

{
	...
	f="$ROOT/dockpipe/workflows/dorkpipe-orchestrator/spec.example.yaml"
	if [[ -f "$f" ]]; then
		sed -n '1,80p' "$f"
	fi
