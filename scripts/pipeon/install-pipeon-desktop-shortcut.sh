...
# Usage: from repo root — bash scripts/install-pipeon-desktop-shortcut.sh
# Windows: use scripts/install-pipeon-desktop-shortcut.ps1 or: make install-pipeon-shortcut-windows
set -euo pipefail

REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
ICON_PNG="${REPO}/contrib/pipeon-vscode-extension/images/icon.png"
ICON_SVG="${REPO}/templates/core/assets/images/code-server/favicon.svg"
LAUNCH="${REPO}/scripts/pipeon-code-server-launch.sh"
