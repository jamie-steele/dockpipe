...
- **Terraform** (`terraform` on `PATH`) — runs **`shipyard/workflows/r2-publish/terraform`** (or `R2_TERRAFORM_DIR`) before upload
...
| `R2_TERRAFORM_DIR` | *(auto)* | Path to Terraform module (default: `shipyard/workflows/r2-publish/terraform` or `templates/r2-publish/terraform` under workdir). |
