...
DEB_GLOB="${REPO_ROOT}/packaging/build/dockpipe_*_all.deb"
...
if [[ ! -f "${DEB}" ]]; then
  echo "test_deb_install FAIL: No .deb found at packaging/build/dockpipe_*_all.deb (run ./packaging/build-deb.sh first)"
  exit 1
fi
