...
- **Go tests:** do not hardcode a developer’s machine path (e.g. `/home/you/...`). Use **`testRepoRoot(t)`** (`lib/dockpipe/application/repo_root_test.go`) for the real repo layout, **`t.TempDir()`** + stubs, or **`DOCKPIPE_REPO_ROOT`** — CI clones to arbitrary directories. Bundled assets are embedded; for tests that materialize the bundle without writing to the real user cache, set **`DOCKPIPE_BUNDLED_CACHE`** to **`t.TempDir()`** (see **`TestRepoRootMaterializesBundledTemplates`**).
